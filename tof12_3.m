function tof12_3(Xedges,Yedges,t3_gate,t12_gate,t0,k0,frag,sct_peak_tof_1,sct_peak_tof_23)

Xedges = min(t12_gate(:,1)):5:max(t12_gate(:,1));
Yedges = min(t3_gate(:,1)):5:max(t3_gate(:,1));

count = histcounts2(t12_gate(:,1),t3_gate(:,1),Xedges,Yedges);
...
xlabel('TOF_1 +TOF_2 /ns', 'FontWeight', 'normal','FontName', 'Arial');
ylabel(' TOF_3/ns', 'FontWeight', 'normal','FontName', 'Arial');
%clims=[1 7];
...
[charge_z;frag_m;frag_m_z];
 ...
 for i=1:length(frag_m_z);
    yline(  k0*(frag_m_z(i))^0.5+t0, '--k', frag_m_z_str(i),'LabelVerticalAlignment' ,'bottom','fontweight','bold','fontsize',10);
 end

ii=8;
jj=3;
xline(  k0*( (frag_m_z(ii) ))^0.5+t0 + k0*( (frag_m_z(jj) ))^0.5+t0, '--k', strcat(frag_m_z_str(ii), '+', frag_m_z_str(jj)),'fontweight','bold','fontsize',30 );

% ii=3;
% jj=3;
% yline(  k0*( (frag_m_z(ii) ))^0.5+t0 + k0*( (frag_m_z(jj) ))^0.5+t0, '--k', strcat(frag_m_z_str(ii), '+', frag_m_z_str(jj)),'fontweight','bold','fontsize',30);
